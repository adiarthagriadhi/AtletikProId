...
// Paket Premium untuk ATLET MANDIRI — terpisah dari paket pelatih di atas
// supaya harga & daftar paket pelatih (openPlanPicker di app.js) tidak berubah.
const ATHLETE_PLANS = {
  monthly: {
    id: 'monthly',
    name: 'Premium Bulanan',
    itemName: 'Atletik Pro Id — Premium Atlet 30 Hari',
    defaultPrice: 29000,
    envKey: 'ATHLETE_MONTHLY_PRICE_IDR',
    durationDays: 30,
  },
  annual: {
    id: 'annual',
    name: 'Premium Tahunan',
    itemName: 'Atletik Pro Id — Premium Atlet 1 Tahun',
    defaultPrice: 199000,
    envKey: 'ATHLETE_ANNUAL_PRICE_IDR',
    durationDays: 365,
  },
};

function getAthletePlan(planId) {
  return ATHLETE_PLANS[planId] || null;
}

function getAthletePriceIdr(planId) {
  const plan = getAthletePlan(planId) || ATHLETE_PLANS.monthly;
  const n = Number(process.env[plan.envKey]);
  return Number.isFinite(n) && n >= 1000 ? Math.round(n) : plan.defaultPrice;
}

function listAthletePlansPublic() {
  return Object.values(ATHLETE_PLANS).map((p) => ({
    id: p.id,
    name: p.name,
    priceIdr: getAthletePriceIdr(p.id),
...
/** order_id atlet mandiri: ATPRO-AM-{athleteUserId}-{ts} / ATPRO-AA-... */
function makeAthleteOrderId(athleteUserId, planId) {
  const p = planId === 'annual' ? 'AA' : 'AM';
  return `ATPRO-${p}-${athleteUserId}-${Date.now()}`;
}

function parseOrderId(orderId) {
  const a = /^ATPRO-(AM|AA)-(\d+)-(\d+)$/.exec(String(orderId || ''));
  if (a) {
    return { accountType: 'athlete', planId: a[1] === 'AA' ? 'annual' : 'monthly', athleteUserId: Number(a[2]), userId: null };
  }
...
async function createSnapToken({ orderId, amount, customer, planId, item, finishPath }) {
  if (!isConfigured()) {
  ...
  const plan = item || getPlan(planId) || PLANS.annual;
  const body = {
    transaction_details: {
      order_id: orderId,
      gross_amount: amount,
  ...
  if (process.env.APP_PUBLIC_URL) {
    body.callbacks = {
      finish: `${process.env.APP_PUBLIC_URL.replace(/\/$/, '')}${finishPath || '/'}?bayar=selesai`,
    };
  }
